import { FileParse } from '../fileParserService';
...
export class ImportService<T> {
  constructor(
    ...
    private readonly fileParse: FileParse<T>
  ) {}
  ...
  async parseUploadedFile(fileName: string): Promise<T[]> {
    ...
    const parsedFile = await this.fileParse.parseFileStream(fileStream);
    try {
